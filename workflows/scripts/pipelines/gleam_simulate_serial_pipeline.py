...
from workflows.serial.simulation.simulation_serial import simulate_serial_workflow, corrupt_serial_workflow
...
if __name__ == '__main__':
    ...
    logging.info("Starting gleam_simulate_serial_pipeline")
    
    serial.set_client(get_dask_Client())
    serial.run(init_logging)
    ...
    vis_list = simulate_serial_workflow('LOWBD2',
                                            rmax=rmax,
                                            frequency=frequency,
    ...
    vis_list = serial.compute(vis_list, sync=True)
    ...
    gleam_model = [serial.execute(create_low_test_image_from_gleam)(npixel=npixel,
    ...
    gleam_model = serial.compute(gleam_model, sync=True)
    gleam_skymodel = SkyModel(images=gleam_model)
    export_skymodel_to_hdf5(gleam_skymodel, 'gleam_simulation_skymodel.hdf')
    future_gleam_model = serial.scatter(gleam_model)
    
    # In[ ]:
    
    log.info('About to run predict to get predicted visibility')
    future_vis_graph = serial.scatter(vis_list)
    predicted_vislist = predict_list_serial_workflow(future_vis_graph, gleam_model, context='wstack', vis_slices=vis_slices)
    corrupted_vislist = corrupt_serial_workflow(predicted_vislist, phase_error=1.0)
    log.info('About to run corrupt to get corrupted visibility')
    corrupted_vislist = serial.compute(corrupted_vislist, sync=True)
