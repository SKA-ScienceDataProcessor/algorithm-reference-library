...
def predict_arlexecute_workflow(vis_list, model_imagelist, vis_slices=1, facets=1, context='2d', **kwargs):
...
def invert_arlexecute_workflow(vis_list, template_model_imagelist, dopsf=False, normalize=True,
...
def residual_arlexecute_workflow(vis, model_imagelist, context='2d', **kwargs):
    ...
    :return:
    """
    model_vis = zero_vislist_arlexecute_workflow(vis)
    model_vis = predict_arlexecute_workflow(model_vis, model_imagelist, context=context, **kwargs)
    residual_vis = subtract_vislist_arlexecute_workflow(vis, model_vis)
    return invert_arlexecute_workflow(residual_vis, model_imagelist, dopsf=False, normalize=True, context=context,
                                      **kwargs)


def restore_arlexecute_workflow(model_imagelist, psf_imagelist, residual_imagelist, **kwargs):
...
def deconvolve_arlexecute_workflow(dirty_list, psf_list, model_imagelist, prefix='', **kwargs):
    ...
    def deconvolve(dirty, psf, model, facet, gthreshold):
        ...
        if this_peak > 1.1 * gthreshold:
            log.info("deconvolve_arlexecute_workflow %s: cleaning - peak %.6f > 1.1 * threshold %.6f" % (lprefix, this_peak,
                                                                                         gthreshold))
            ...
            if result.data.shape[0] == model.data.shape[0]:
                ...
                log.warning("deconvolve_arlexecute_workflow %s: Initial model %s and clean result %s do not have the same shape" %
            ...
            log.info("deconvolve_arlexecute_workflow %s: Not cleaning - peak %.6f <= 1.1 * threshold %.6f" % (lprefix, this_peak,
                                                                                                  gthreshold))
...
def deconvolve_channel_arlexecute_workflow(dirty_list, psf_list, model_imagelist, subimages, **kwargs):
...
def weight_arlexecute_workflow(vis_list, model_imagelist, weighting='uniform', **kwargs):
...
def zero_vislist_arlexecute_workflow(vis_list):
...
def subtract_vislist_arlexecute_workflow(vis_list, model_vislist):
