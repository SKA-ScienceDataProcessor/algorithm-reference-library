...
//  adv->cellsize = 0;
  ...
  int npixel, vis_slices;// = malloc(sizeof(int));
...
//			(uintptr_t)gleam_model, sizeof(Image));
	starpu_variable_data_register(&vis_slices_h, STARPU_MAIN_RAM,
  	(uintptr_t)&(vis_slices), sizeof(int));
  ...
  predict_function_blockvis_task->handles[3] = vis_slices_h;
  predict_function_blockvis_task->modes[3] = STARPU_R;
...
//  
  ...
  simulate_gaintable_task->modes[1] = STARPU_W;
  simulate_gaintable_task->synchronous = 1;
