...
def predict_list_mpi_workflow(vis_list, model_imagelist, vis_slices=1,
   ...
   """
    ...
    # Each MPI process Loops over all his frequency windows
    for freqwin, vis_list in enumerate(sub_vis_list):
        # Create the graph to divide an image into facets. This is by reference.
        facet_lists = image_scatter_facets(sub_model_imagelist[freqwin], facets=facets)
...
def invert_list_mpi_workflow(vis_list, template_model_imagelist, dopsf=False, normalize=True,
   ...
   # same as normalizing the whole
    ...
    for freqwin, vis_list in enumerate(sub_vis_list):
        # Create the graph to divide an image into facets. This is by reference.
        facet_lists = image_scatter_facets(sub_template_model_imagelist[
                                               freqwin],
                                           facets=facets)
        # Create the graph to divide the visibility into slices. This is by copy.
        ...
        for sub_vis_list in sub_vis_lists:
            ...
            vis_results.append(gather_image_iteration_results(facet_vis_results,
                                                              sub_template_model_imagelist[freqwin]))
        # sum_invert_results has the normalization    
        results_vislist.append(sum_invert_results(vis_results))
    results_vislist=comm.gather(results_vislist,root=0)
    if rank == 0:
