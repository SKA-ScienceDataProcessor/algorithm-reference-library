...
def create_invert_graph(vis_graph_list, template_model_graph: delayed, vis_slices=None,
                        dopsf=False, normalize=True, context='2d',
                        **kwargs) -> delayed:
    ...
    """
    invert = imaging_context(context)['invert']
    scatter = imaging_context(context)['scatter']
...
def create_invert_facet_graph(vis_graph_list, template_model_graph: delayed, dopsf=False,
                              normalize=True, facets=1, context='2d',
                              **kwargs) -> delayed:
   ...
   """
    ...
    def gather_invert_results(results, template_model, facets, **kwargs):
        ...
        image_results = image_gather_facets([result[0] for result in results],
                                            image_results, facets=facets)
    ...
    model_graphs = \
        delayed(image_scatter_facets, nout=facets ** 2, pure=True)(template_model_graph,
                                                                   facets=facets)
    
    # For each facet, invert over the vis_graph
    results = [create_invert_graph(vis_graph_list, model_graph, dopsf=dopsf, normalize=normalize,
    ...
    return delayed(gather_invert_results, nout=2, pure=True)(results, template_model_graph,
                                                             facets=facets, **kwargs)


def create_predict_graph(vis_graph_list, model_graph: delayed, vis_slices=None,
                         context='wstack_single', **kwargs):
    ...
    :return: List of vis_graphs
   """
    predict = imaging_context(context)['predict']
    scatter = imaging_context(context)['scatter']
    gather = imaging_context(context)['gather']
    ...
    if scatter is not None and gather is not None and vis_slices is not None:
        for vis_graph in vis_graph_list:
            scatter_vis_graphs = \
                delayed(scatter, nout=vis_slices)(vis_graph, vis_slices=vis_slices, **kwargs)
            predict_list = list()
            for scatter_vis_graph in scatter_vis_graphs:
                predict_list.append(delayed(predict_and_accumulate, pure=True,
                                            nout=1)(scatter_vis_graph, model_graph, **kwargs))
            predicted_vis_list.append(delayed(gather, nout=1)(predict_list, vis_graph,
                                                              vis_slices=vis_slices,
                                                              **kwargs))
    else:
        return [delayed(predict_and_accumulate, pure=True, nout=1)(v, model_graph, **kwargs)
                for v in vis_graph_list]
    
    return predicted_vis_list


def create_predict_facet_graph(vis_graph_list, model_graph: delayed, context='wstack_single',
                               facets=2, **kwargs):
    ...
    for vis_graph in vis_graph_list:
        for ifacet, facet_model_graph in enumerate(facet_model_graphs):
            ...
            if ifacet == 0:
                accumulate_vis_graph = delayed(predict_facets_and_accumulate, pure=True,
                                               nout=1)(vis_graph, facet_model_graph, **kwargs)
            else:
        ...
        for scatter_vis_graph in scatter_vis_graphs:
            for ifacet, facet_model_graph in enumerate(facet_model_graphs):
                accumulate_vis_graph = delayed(predict_facets_and_accumulate,
                                               pure=True, nout=1)(scatter_vis_graph,
                                                                  facet_model_graphs[ifacet],
                                                                  **kwargs)
                accumulate_vis_graphs.append(accumulate_vis_graph)
        
        predicted_vis_list.append(delayed(gather, nout=1)(accumulate_vis_graphs, vis_graph,
                                                          vis_slices=vis_slices, **kwargs))
    
    return predicted_vis_list


def create_residual_graph(vis_graph_list, model_graph: delayed, vis_slices,
    ...
    if scatter is not None and gather is not None:
        def zerovis(vis):
            if vis is not None:
                zerovis = copy_visibility(vis)
                zerovis.data['vis'][...] = 0.0
        ...
        model_vis_graph_list = create_predict_graph(model_vis_graph_list, model_graph, context=context,
                                                    **kwargs)
        residual_vis_graph_list = create_subtract_vis_graph_list(vis_graph_list,
                                                                 model_vis_graph_list)
        return create_invert_graph(residual_vis_graph_list, model_graph, dopsf=False,
                                   normalize=True, context=context, **kwargs)
...
def create_deconvolve_graph(dirty_graph: delayed, psf_graph: delayed, model_graph: delayed, facets=1,
                            **kwargs) -> \
        delayed:
