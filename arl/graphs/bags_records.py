...
def predict_record(record, model, context, **kwargs):
    ...
    newrecord['vis'] = predict(newvis, model, context=context, **kwargs)
    return newrecord


def invert_record(record, model, dopsf, context, **kwargs):
    ...
    for key in record.keys():
        if key != 'vis':
            newrecord[key] = record[key]
    newrecord['image'] = invert(vis, model, dopsf, **kwargs)
    return newrecord
...
def invert_bag(vis_bag, model_bag, dopsf=False, context='2d', key='freqwin', **kwargs) -> bag:
    ...
    return vis_bag \
        ...
        .flatten()\
        .map(invert_record, model_bag, dopsf=dopsf, context=context, **kwargs) \
...
def predict_bag(vis_bag, model_bag, context='2d', key='freqwin', **kwargs) -> bag:
    ...
    return vis_bag\
        .map(scatter_record, context, **kwargs) \
        .flatten() \
        .map(predict_record, model_bag, context, **kwargs) \
