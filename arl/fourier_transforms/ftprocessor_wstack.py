...
def predict_wstack(vis, model, **kwargs):
    ...
    :param wstack: wstack in seconds. If 'auto' will find plausible value
    ...
    kwargs['wstack'] = get_parameter(kwargs, "wstep", advice['w_sampling_primary_beam'])

    return predict_with_vis_iterator(vis, model, vis_iter=vis_wstack_iter,
                                     predict=predict_wstack_single, **kwargs)


def predict_wstack_single(vis, model, **kwargs):
...
def invert_wstack(vis, im, dopsf=False, normalize=True, **kwargs):
    ...
    :param wstack: wstack in wavelenngths. If None or omitted will find plausible value
    :param nprocessor: Number of processors to be used (1)
    ...
    delA = get_parameter(kwargs, 'wloss', 0.02)
    advice = advise_wide_field(vis, delA)
    wstack = get_parameter(kwargs, "wstep", advice['w_sampling_primary_beam'])
...
def invert_wstack_single(vis, im, dopsf, normalize=True, **kwargs):
