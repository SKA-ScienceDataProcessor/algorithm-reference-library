...
from arl.skycomponent.operations import copy_skymodel

log = logging.getLogger(__name__)


def initialise_skymodel_cal_skymodel(vis: BlockVisibility, images=None, components=None, **kwargs):
    """Create the skymodel
    ...
    return SkyModel(images=images, components=components, gt=gt)


def skymodel_cal_fit_component(vis, ssm, gain=0.1, method='fit', **kwargs):
    ...
    :param ssm: ssm element being fit
    ...
    new_comp = copy_skycomponent(ssm[0])
    if method == 'sum':
        new_flux, _ = sum_visibility(cvis, new_comp.direction)
        new_comp.flux = gain * new_flux + (1.0 - gain) * ssm[0].flux
    else:
        new_comp, _ = fit_visibility(cvis, new_comp)
        new_comp.flux = gain * new_comp.flux + (1.0 - gain) * ssm[0].flux

    return new_comp

def skymodel_cal_fit_gaintable(evis, ssm, gain=0.1, niter=3, tol=1e-3, **kwargs):
    ...
    """
    previous_gt = copy_gaintable(ssm[1])
    gt = copy_gaintable(ssm[1])
    model_vis = copy_visibility(evis, zero=True)
    model_vis = predict_skycomponent_visibility(model_vis, ssm[0])
...
def skymodel_cal_e_step(vis: BlockVisibility, evis_all: BlockVisibility, ssm, **kwargs):
    ...
    :param ssm: ssm element being fit
    :param kwargs:
    :return: Data model (i.e. visibility) for this ssm
    """
    evis = copy_visibility(evis_all)
    tvis = copy_visibility(vis, zero=True)
    tvis = predict_skycomponent_visibility(tvis, ssm[0])
    tvis = apply_gaintable(tvis, ssm[1])
    evis.data['vis'][...] = tvis.data['vis'][...] + vis.data['vis'][...] - evis_all.data['vis'][...]
    return evis


def skymodel_cal_e_all(vis: BlockVisibility, sm):
    """Calculates E step in equation A12
    ...
    tvis = copy_visibility(vis, zero=True)
    for i, ssm in enumerate(sm.components):
        tvis.data['vis'][...] = 0.0
        tvis = predict_skycomponent_visibility(tvis, ssm[0])
        tvis = apply_gaintable(tvis, ssm[1])
...
def skymodel_cal_m_step(evis: BlockVisibility, ssm, **kwargs):
    ...
    :return:
    """
    return (skymodel_cal_fit_component(evis, ssm, **kwargs),
            skymodel_cal_fit_gaintable(evis, ssm, **kwargs))


def skymodel_cal_solve(vis, components, niter=10, tol=1e-8, gain=0.25, callback=None, **kwargs):
    """ Solve
    ...
    """
    skymodel = initialise_skymodel_cal_skymodel(vis, images=None, components=components, **kwargs)
    
    for iter in range(niter):
        new_skymodel = SkyModel()
        evis_all = skymodel_cal_e_all(vis, skymodel)
        print("Iteration %d" % (iter))
        for window_index, ssm in enumerate(skymodel.components):
            evis = skymodel_cal_e_step(vis, evis_all, ssm, gain=gain, **kwargs)
            new_ssm = skymodel_cal_m_step(evis, ssm, **kwargs)
            new_skymodel.append_component(new_ssm[0], new_ssm[1])
            
            if callback is not None:
                callback(iter, skymodel)
                
            flux = new_ssm[0].flux[0, 0]
            qa = qa_gaintable(new_ssm[1])
            ...
            print("\t Window %d, flux %s, residual %.3f, rms phase %.3f" % (window_index, str(flux), residual,
                                                                            rms_phase))
        skymodel = copy_skymodel(new_skymodel)
    
    residual_vis = copy_visibility(vis)
    final_vis = skymodel_cal_e_all(vis, skymodel)
    residual_vis.data['vis'][...] = vis.data['vis'][...] - final_vis.data['vis'][...]
    return skymodel, residual_vis
