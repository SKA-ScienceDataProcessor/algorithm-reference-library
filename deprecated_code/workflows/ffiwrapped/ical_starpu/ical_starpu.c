...
//  SVDR(advise_wide_field_h, 2, adv-> sizeof(ARLadv->ce)); 
  ...
  advise_wide_field_task->modes[2] = STARPU_W;
...
//	printf("Vis_slices = %d,  npixel = %d, cellsize = %e\n", adv->vis_slices, adv->npixel, adv->cellsize);
	...
	//
	//helper_get_image_shape_multifreq(lowconfig, adv->cellsize, adv->npixel, shape[0]);
...
//  SVDR(helper_get_image_shape_multifreq_h, 3, &shape, sizeof(shape[0]));	
  ...
  starpu_data_handle_t shape_h;  
	starpu_variable_data_register(&cellsize_h, STARPU_MAIN_RAM,
			(uintptr_t)&(adv->cellsize), sizeof(double));
  ...
  helper_get_image_shape_multifreq_task->modes[0] = STARPU_RW;
  helper_get_image_shape_multifreq_task->handles[1] = cellsize_h;
...
//	gleam_model = allocate_image(shape);
  starpu_data_handle_t allocate_image_h[2];
  SVDR(allocate_image_h, 0, shape, sizeof(shape[0]));
  SVDR(allocate_image_h, 1, &gleam_model, sizeof(Image));
  struct starpu_task *allocate_image_task = create_task(&allocate_image_cl, allocate_image_h);

  starpu_task_declare_deps_array(allocate_image_task, 1, &helper_get_image_shape_multifreq_task);// &advise_wide_field_task);
...
//  starpu_task_declare_deps_array(create_low_test_image_from_gleam_task, 1, &advise_wide_field_task);
  starpu_task_declare_deps_array(create_low_test_image_from_gleam_task, 1, &allocate_image_task);
  create_low_test_image_from_gleam_task->synchronous = 1; 
  status_code = starpu_task_submit(create_low_test_image_from_gleam_task);
	printf("Nrec = %d %d\n", lowconfig->nrec, adv->npixel);
...
//	arl_invert_function_blockvis(lowconfig, vt_gt, model, adv->vis_slices, dirty);
  ...
  //invert_function_blockvis_task->synchronous = 1;
...
//  starpu_data_handle_t set_visibility_data_to_zero_h[2];
  ...
  set_visibility_data_to_zero_task = create_task(&set_visibility_data_to_zero_cl, set_visibility_data_to_zero_h);
...
//  starpu_data_handle_t predict_function_ical_h[6];
  ...
  predict_function_ical_task = create_task(&predict_function_ical_cl, predict_function_ical_h);
