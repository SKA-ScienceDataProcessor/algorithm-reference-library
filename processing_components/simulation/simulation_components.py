...
from operations import apply_gaintable, create_gaintable_from_blockvisibility
...
def simulate_component(config='LOWBD2',
    ...
    if order == 'time':
        log.debug("simulate_component: Simulating distribution in %s" % order)
        vis_list = list()
        ...
        log.debug("simulate_component: Simulating distribution in time and frequency")
        vis_list = list()
        ...
        log.debug("simulate_component: Simulating into single %s" % format)
        vis_list = list()
...
def corrupt_component(vis_list, gt_graph=None, **kwargs):
