...
from arl.graphs.graphs import create_zero_vis_graph_list, create_subtract_vis_graph_list, \
    create_deconvolve_facet_graph, create_deconvolve_channel_graph, \
    create_predict_graph, create_invert_graph, create_selfcal_graph_list, create_residual_graph
...
class TestDaskGraphs(unittest.TestCase):
    ...
    def ingest_visibility(self, freq=1e8, chan_width=1e6, times=None, reffrequency=None, add_errors=False):
        ...
        export_image_to_fits(model, '%s/test_graphs_imaging_model.fits' % (self.results_dir))
        if add_errors:
    ...
    def test_predict_wstack_graph(self):
        ...
        zero_vis_graph_list = create_zero_vis_graph_list(self.vis_graph_list)
        predicted_vis_graph_list = create_predict_graph(zero_vis_graph_list, flux_model_graph,
                                                        vis_slices=self.vis_slices, context='wstack')
        residual_vis_graph_list = create_subtract_vis_graph_list(self.vis_graph_list,
                                                                 predicted_vis_graph_list)
        if self.compute:
            qa = qa_visibility(self.vis_graph_list[0].compute())
    ...
    def test_predict_timeslice_graph(self):
        ...
        zero_vis_graph_list = create_zero_vis_graph_list(self.vis_graph_list)
        predicted_vis_graph_list = create_predict_graph(zero_vis_graph_list, flux_model_graph,
                                                        vis_slices=3, context='timeslice')
        residual_vis_graph_list = create_subtract_vis_graph_list(self.vis_graph_list,
                                                                 predicted_vis_graph_list)
        if self.compute:
            qa = qa_visibility(self.vis_graph_list[0].compute())
    ...
    def test_predict_timeslice_graph_wprojection(self):
        ...
        predicted_vis_graph_list = create_predict_graph(zero_vis_graph_list, flux_model_graph,
                                                        ...
                                                        wstep=4.0, context='timeslice', remove=True,)
        residual_vis_graph_list = create_subtract_vis_graph_list(self.vis_graph_list,
                                                                 predicted_vis_graph_list)
        if self.compute:
            qa = qa_visibility(self.vis_graph_list[0].compute())
    ...
    def test_predict_facet_wstack_graph(self):
        flux_model_graph = delayed(self.get_LSM)(self.vis_graph_list[self.nvis // 2], flux=100.0)
        zero_vis_graph_list = create_zero_vis_graph_list(self.vis_graph_list)
        predicted_vis_graph_list = create_predict_graph(zero_vis_graph_list,
                                                        flux_model_graph,
                                                        facets=2,
                                                        vis_slices=self.vis_slices,
    ...
    def test_predict_wstack_graph_wprojection(self):
        ...
        predicted_vis_graph_list = create_predict_graph(zero_vis_graph_list, flux_model_graph,
                                                        ...
                                                        kernel='wprojection', context='wstack')
        residual_vis_graph_list = create_subtract_vis_graph_list(self.vis_graph_list,
                                                                 predicted_vis_graph_list)
        if self.compute:
            qa = qa_visibility(self.vis_graph_list[0].compute())
    ...
    def test_predict_facet_timeslice_graph_wprojection(self):
        ...
        predicted_vis_graph_list = create_predict_graph(zero_vis_graph_list, flux_model_graph,
                                                        ...
                                                        context='facets_timeslice', remove=True)
    ...
    def test_predict_facet_graph(self):
        ...
        zero_vis_graph_list = create_zero_vis_graph_list(self.vis_graph_list)
        predicted_vis_graph_list = create_predict_graph(zero_vis_graph_list, flux_model_graph,
                                                        facets=self.facets, context='facets')
        residual_vis_graph_list = create_subtract_vis_graph_list(self.vis_graph_list,
                                                                 predicted_vis_graph_list)
        if self.compute:
            qa = qa_visibility(self.vis_graph_list[0].compute())
    ...
    def test_invert_graph(self):
        ...
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_graph_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
            
            assert numpy.abs(qa.data['max'] - 95.6) < 2.0, str(qa)
            assert numpy.abs(qa.data['min'] + 4.7) < 2.0, str(qa)
    ...
    def test_invert_graph_wprojection(self):
        ...
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_graph_wprojection_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
            
            assert numpy.abs(qa.data['max'] - 99.18) < 2.0, str(qa)
            assert numpy.abs(qa.data['min'] + 4.9) < 2.0, str(qa)
    
    def test_invert_facet_graph(self):
        
        dirty_graph = create_invert_graph(self.vis_graph_list, self.model_graph, context='facets',
                                          dopsf=False, normalize=True, facets=4)
        
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_facet_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
            
            assert numpy.abs(qa.data['max'] - 101.7) < 2.0, str(qa)
            assert numpy.abs(qa.data['min'] + 3.5) < 2.0, str(qa)
    
    def test_invert_wstack_graph(self):
        
        dirty_graph = create_invert_graph(self.vis_graph_list, self.model_graph, context='wstack',
                                          dopsf=False, normalize=True,
                                          vis_slices=self.vis_slices)
        
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_wstack_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
    ...
    def test_invert_timeslice_graph_wprojection(self):
        ...
        dirty_graph = create_invert_graph(self.vis_graph_list, self.model_graph, context='timeslice',
                                          dopsf=False, normalize=True,
                                          vis_slices=3, kernel='wprojection', remove=True,
                                          wstep=10.0)
        
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_timeslice_wprojection_dirty.fits' % (
                self.results_dir))
            qa = qa_image(dirty)
    ...
    def test_invert_timeslice_graph(self):
        ...
        dirty_graph = create_invert_graph(self.vis_graph_list, self.model_graph, context='timeslice',
                                          dopsf=False, normalize=True, vis_slices=3)
        
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_timeslice_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
            assert numpy.abs(qa.data['max'] - 101.7) < 2.0, str(qa)
            assert numpy.abs(qa.data['min'] + 3.5) < 2.0, str(qa)
    ...
    def test_invert_facet_wstack_graph(self):
        ...
        dirty_graph = create_invert_graph(self.vis_graph_list, self.model_graph, context='facets_wstack',
                                          dopsf=False, normalize=True,
                                          vis_slices=self.vis_slices, facets=4)
        
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_facet_wstack_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
    ...
    def test_invert_facet_wstack_graph_wprojection(self):
        ...
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_facet_wstack_wprojection_dirty.fits' % (
                self.results_dir))
            qa = qa_image(dirty)
    ...
    def test_invert_facet_timeslice_graph(self):
        ...
        dirty_graph = create_invert_graph(self.vis_graph_list, self.model_graph, context='facets_timeslice',
                                          dopsf=False, normalize=True,
                                          vis_slices=self.vis_slices, facets=2, remove=True)
        
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_facet_timeslice_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
    ...
    def test_invert_facet_timeslice_graph_wprojection(self):
        ...
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_facet_timeslice_wprojection_dirty.fits' % (
                self.results_dir))
            qa = qa_image(dirty)
    ...
    def test_invert_wstack_graph_wprojection(self):
        ...
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_invert_wstack_wprojection_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
    ...
    def test_residual_facet_graph(self):
        ...
        dirty_graph = create_residual_graph(self.vis_graph_list, self.model_graph, context='facets',
                                            facets=self.facets)
        
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_residual_facet%d.fits' %
    ...
    def test_residual_wstack_graph(self):
        ...
        dirty_graph = create_residual_graph(self.vis_graph_list, self.model_graph, context='wstack',
                                            vis_slices=self.vis_slices)
        
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_residual_wstack_slices%d.fits' %
    ...
    def test_residual_facet_wstack_graph(self):
        ...
        dirty_graph = create_residual_graph(self.vis_graph_list, self.model_graph, context='facets_wstack',
    ...
    def test_residual_wstack_graph_wprojection(self):
        ...
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_residual_wprojection.fits' %
                                 (self.results_dir))
    ...
    def test_deconvolution_facet_graph(self):
        ...
        model_graph = delayed(self.get_LSM)(self.vis_graph_list[self.nvis // 2],
                                            flux=0.0)
        dirty_graph = create_invert_graph(self.vis_graph_list, model_graph, context='wstack',
                                          dopsf=False, vis_slices=self.vis_slices)
        psf_model_graph = delayed(self.get_LSM)(self.vis_graph_list[self.nvis // 2],
                                                flux=0.0)
        psf_graph = create_invert_graph(self.vis_graph_list, psf_model_graph, context='wstack',
                                        vis_slices=self.vis_slices,
                                        dopsf=True)
        ...
        if self.compute:
            ...
            export_image_to_fits(result, '%s/test_graphs_imaging_deconvolution_facets%d.clean.fits' %
                                 (self.results_dir, facets))
            
            qa = qa_image(result)
            
            assert numpy.abs(qa.data['max'] - 100.1) < 2.0, str(qa)
            assert numpy.abs(qa.data['min'] + 1.8) < 2.0, str(qa)
    ...
    def test_deconvolution_channel_graph(self):
        ...
        if self.compute:
            ...
            export_image_to_fits(result, '%s/test_graphs_imaging_deconvolution_channels%d.clean.fits' %
    ...
    def test_selfcal_global_graph(self):
        ...
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_graphs_global_selfcal_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
    ...
    def test_selfcal_nonglobal_graph(self):
        ...
        dirty_graph = create_invert_graph(selfcal_vis_graph_list, self.model_graph, context='wstack',
                                          dopsf=False, normalize=True,
                                          vis_slices=self.vis_slices)
        
        if self.compute:
            dirty, sumwt = dirty_graph[0].compute()
            export_image_to_fits(dirty, '%s/test_graphs_imaging_graphs_nonglobal_selfcal_dirty.fits' % (self.results_dir))
            qa = qa_image(dirty)
