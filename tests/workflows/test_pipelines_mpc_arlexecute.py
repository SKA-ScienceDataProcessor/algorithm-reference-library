...
class TestMPCCAL(ARLExecuteTestCase, unittest.TestCase):
    
    def setUp(self):
        super(TestMPCCAL, self).setUp()
    ...
    def actualSetUp(self, add_errors=False, nfreqwin=7, dospectral=True, dopol=False, zerow=True):
        ...
        if self.persist:
            export_image_to_fits(model, '%s/test_pipelines_mpccal_arlexecute_model.fits' % self.dir)
            export_image_to_fits(self.cmodel, '%s/test_pipelines_mpccal_arlexecute_cmodel.fits' % self.dir)
