...
class TestPipelineGraphs(unittest.TestCase):
    ...
    def test_continuum_imaging_pipeline_wstack(self):
        self.actualSetUp(add_errors=False, zerow=False)
        continuum_imaging_list = \
            continuum_imaging_list_arlexecute_workflow(self.vis_list,
                                                       model_imagelist=self.model_imagelist,
                                                       context='wstack', vis_slices=11,
                                                       algorithm='mmclean', facets=1,
                                                       scales=[0, 3, 10],
                                                       niter=1000, fractional_threshold=0.1, threshold=0.1,
                                                       nmoment=3,
                                                       ...
                                                       deconvolve_facets=4, deconvolve_overlap=32,
                                                       deconvolve_taper='tukey', psf_support=64,
                                                       use_serial_invert=False, use_serial_predict=False)
        clean, residual, restored = arlexecute.compute(continuum_imaging_list, sync=True)
        centre = len(clean) // 2
        if self.persist:
            export_image_to_fits(clean[centre], '%s/test_pipelines_continuum_imaging_pipeline_wstack_arlexecute_clean.fits' %
                                 self.dir)
            export_image_to_fits(residual[centre][0],
                                 '%s/test_pipelines_continuum_imaging_pipeline_wstack_arlexecute_residual.fits' % self.dir)
            export_image_to_fits(restored[centre],
                                 '%s/test_pipelines_continuum_imaging_pipeline_wstack_arlexecute_restored.fits' % self.dir)
